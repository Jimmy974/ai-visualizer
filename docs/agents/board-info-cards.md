...
2. If none remain, query the next 30 days and publish **only the single next event**. Label `range` as that 30-day window (`today` → `today+30`, exclusive end), not as the event's own day.
