...
1. Start the server with no snapshot file and open `/faces/board/index.html`. Request `GET /board-cards`. Pass: the API returns HTTP 200 with version 1, an empty cards list and `Cache-Control: no-store`; the board invites the user to ask Sinner for tasks/calendar without showing fabricated results.
